...
export default function OrdersPage() {
  ...
  const load = async () => {
    const [ro, rl, rp] = await Promise.all([
      ...
      fetch(API_ENDPOINTS.supplier.inventoryLots.getAll, { cache: "no-store", credentials: 'include' }),
    ...
    const allLots = dll.data || [];
    const userId = user?.userId ?? user?.id;
    setLots(userId ? allLots.filter((l) => Number(l.farmerId) === Number(userId)) : []);
  ...
  const loadOrderItems = async (orderId) => {
    ...
    try {
      const r = await fetch(API_ENDPOINTS.supplier.orders.getItems(orderId), { cache: 'no-store', credentials: 'include' });
  ...
  if (authLoading) return <div className="p-6">در حال بررسی دسترسی...</div>;
  if (!allowed) return null;

  return (
    <div className="p-2 sm:p-4">
      <h1 className="text-lg sm:text-xl font-bold mb-4 sr-only">سفارشات مشتری</h1>
