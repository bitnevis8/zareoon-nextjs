...
export default function ProductCategoriesPage() {
  ...
  const load = useCallback(async () => {
    const url = `${API_ENDPOINTS.supplier.productCategories.getAll}?sortBy=${encodeURIComponent(sortBy)}&sortOrder=${encodeURIComponent(sortOrder)}`;
  ...
  const create = async (e) => {
    e.preventDefault();
    setLoading(true);
    await fetch(API_ENDPOINTS.supplier.productCategories.create, {
      method: "POST",
  ...
  const remove = async (id) => {
    await fetch(API_ENDPOINTS.supplier.productCategories.delete(id), { method: "DELETE" });
    load();
  };
