...
import {
  ...
  SERVICE_TYPE_LABELS_FA,
  TRANSPORT_MODE_LABELS_FA,
} from "../../data/serviceRequestForms";
...
function formatDetailValue(key, value) {
  ...
  if (key === "transportMode") return TRANSPORT_MODE_LABELS_FA[value] || value;
  if (key === "paymentMethod") return PAYMENT_METHOD_LABELS_FA[value] || value;
  return String(value);
}

export default function ServiceRequestsDashboardContent() {
  const auth = useAuth();
  ...
  const loadRequests = async () => {
    try {
      setLoading(true);
      const query = typeFilter ? `?serviceType=${encodeURIComponent(typeFilter)}` : "";
      const response = await fetch(`${API_ENDPOINTS.serviceRequests.getAll}${query}`, {
        credentials: "include",
      });
      if (response.ok) {
  ...
  const saveUpdate = async () => {
    ...
    setSaving(true);
    try {
      const response = await fetch(API_ENDPOINTS.serviceRequests.updateStatus(selected.id), {
        method: "PATCH",
  ...
  const detailEntries = useMemo(() => {
    ...
    return Object.entries(selected.details)
      .map(([key, value]) => ({
        key,
        label: DETAIL_FIELD_LABELS_FA[key] || key,
    ...
    <div className="container mx-auto px-2 sm:px-4 py-4 sm:py-8">
      <div className="mb-6 flex flex-wrap items-end justify-between gap-3">
        ...
        <label className="text-sm">
          <span className="block text-gray-600 mb-1">فیلتر خدمت</span>
          <select
            value={typeFilter}
            onChange={(e) => setTypeFilter(e.target.value)}
            className="rounded-lg border border-gray-200 px-3 py-2 min-w-[12rem]"
          >
            <option value="">همه خدمات</option>
            {Object.entries(SERVICE_TYPE_LABELS_FA).map(([key, label]) => (
          ...
          <table className="min-w-full divide-y divide-gray-200 text-sm">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-4 py-3 text-right font-medium text-gray-600">خدمت</th>
              ...
              {requests.length === 0 ? (
                ...
                requests.map((item) => (
                  <tr key={item.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3">{SERVICE_TYPE_LABELS_FA[item.serviceType] || item.serviceType}</td>
            ...
            requests.map((item) => (
              ...
              >
                ...
                <div className="mt-1 text-xs text-gray-500">
                  {SERVICE_TYPE_LABELS_FA[item.serviceType] || item.serviceType}
                </div>
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
          ...
          <div className="relative w-full sm:max-w-lg max-h-[90vh] overflow-y-auto bg-white rounded-t-2xl sm:rounded-2xl shadow-xl p-5 space-y-4">
            ...
            <dl className="space-y-2 text-sm">
              <div>
                <dt className="text-gray-500">نوع خدمت</dt>
                <dd className="font-medium">
                  {SERVICE_TYPE_LABELS_FA[selected.serviceType] || selected.serviceType}
                </dd>
              </div>
              <div>
              ...
              {selected.company ? (
                <div>
                  <dt className="text-gray-500">شرکت</dt>
              ...
              {selected.tradeType ? (
                <div>
                  <dt className="text-gray-500">نوع معامله</dt>
                  <dd>{TRADE_LABELS[selected.tradeType] || selected.tradeType}</dd>
                </div>
              ) : null}
              {selected.productDescription ? (
                <div>
                  <dt className="text-gray-500">کالا / خدمات</dt>
                  <dd>{selected.productDescription}</dd>
                </div>
              ) : null}
              {selected.estimatedAmount ? (
                <div>
                  <dt className="text-gray-500">مبلغ تقریبی</dt>
                  <dd dir="ltr">
                    {selected.estimatedAmount} {selected.currency}
                  </dd>
                </div>
              ) : null}
              ...
              {selected.notes ? (
                <div>
                  <dt className="text-gray-500">توضیحات</dt>
