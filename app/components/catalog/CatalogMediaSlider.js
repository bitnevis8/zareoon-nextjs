...
export function buildMediaSlides({ product, coverUrl, media = [], title = "" }) {
  ...
  if (product) {
    const productUrl = resolveMediaUrl(product.imageUrl);
    slides.push({ id: "product-cover", kind: "product", product, alt: title });
    seen.add("product-cover");
    if (productUrl) seen.add(productUrl);
  } else if (coverUrl) {
    const resolvedCover = resolveMediaUrl(coverUrl);
    slides.push({
      id: "cover",
      kind: "url",
      url: resolvedCover || coverUrl,
      mimeType: "image/jpeg",
      alt: title,
    });
    if (resolvedCover) seen.add(resolvedCover);
    seen.add(coverUrl);
  }
