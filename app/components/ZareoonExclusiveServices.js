...
function CategoryBrandMark({ item, size = "md", alt = "", tone = "amber" }) {
  ...
  const borderTone = tone === "emerald" ? "border-emerald-200/90" : "border-amber-200/90";

  if (logo) {
    return (
      <div className={`relative shrink-0 overflow-hidden border bg-white shadow-sm ${borderTone} ${sizeClass}`}>
        <Image
          src={logo}
          alt={alt || item.title}
          fill
          sizes={size === "wide" ? "112px" : "64px"}
          className={`object-contain ${size === "wide" ? "p-0.5 sm:p-1" : "p-1"}`}
        />
      </div>
    );
  }
