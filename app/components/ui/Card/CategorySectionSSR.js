...
export default async function CategorySectionSSR({ category, articles }) {
  const t = await getTranslations('home.categorySection');
  ...
  return (
    <Card title={category.name} className="mb-8">
                  ...
                  <div className="flex flex-col sm:flex-row gap-4 items-center">
                    ...
                    <div className="flex-1 flex flex-col justify-between h-full">
                      ...
                      <div className="flex items-center justify-between text-xs text-gray-500 mt-auto">
                        <span>{timeAgo(first.publishedAt, t)}</span>
                      </div>
                    </div>
                  </div>
                </div>
      ...
      <div className="mt-4 text-center">
        ...
        >
          {t('viewAllNews', { title: category.name })}
        </Link>
      </div>
    </Card>
  );
