...
import { publicAbsoluteUrl, publicDisplayUrl } from "@/app/utils/productQrUrl";
import { PRODUCT_QR_PRINT_SIZES, downloadProductQrPrint } from "@/app/utils/productQrPrint";

/**
 * QR عمومی محصول / لندینگ — مشابه QR فروشگاه
 */
export default function ProductPageQrCode({
  ...
  const pageUrl = useMemo(() => publicAbsoluteUrl(pathOrUrl), [pathOrUrl]);
  const displayUrl = useMemo(() => publicDisplayUrl(pathOrUrl), [pathOrUrl]);
  ...
  return (
    <>
      <div className={`rounded-xl border border-slate-200 bg-slate-50/80 p-3 ${className}`}>
        <p className="mb-2 text-[11px] font-semibold text-slate-600">{heading}</p>
        <button
          type="button"
      ...
      <Modal isOpen={open} onClose={() => !busyId && setOpen(false)} title="دانلود QR کد قابل چاپ">
        <div className="space-y-4">
          ...
          <p className="text-xs leading-relaxed text-slate-600">
            اندازه مناسب چاپ را انتخاب کنید. فایل PDF (یا PNG) دانلود می‌شود. با تغییر آدرس صفحه، این QR هم به‌روز
            می‌شود.
          </p>

          <ul className="space-y-2">
            {PRODUCT_QR_PRINT_SIZES.map((size) => {
