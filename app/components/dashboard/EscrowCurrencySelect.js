...
import { ESCROW_CURRENCIES, getEscrowCurrency } from "@/app/utils/escrowCurrencies";

export default function EscrowCurrencySelect({ value, onChange, className = "" }) {
  const selected = getEscrowCurrency(value);

  return (
    <div className={className}>
      <label className="block">
        <span className="text-xs font-medium text-slate-600">ارز معامله</span>
        <select
          value={value || "IRR"}
          ...
          aria-label="ارز معامله"
        >
          {ESCROW_CURRENCIES.map((c) => (
